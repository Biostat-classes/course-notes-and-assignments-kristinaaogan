...
The : means create a list of numbers starting from the bottom number to
the top number that includes all integers in the interval.
...
Length of x returns the number of elements in a vector.
...
The square brackets allow you to work with a certain set of values from
the vector.
...
The brackets select a certain group of numbers from the vector.

#### 3

Is `-1:2` the same as `(-1):2` or

```{r}

```

? INSERT AN R CHUNK HERE AND RUN EACH OF THESE COMMANDS. Then state what
each of these does.
...
The first command gives us a sequence of numbers from -1 to 2. The
second command does the same thing. However, the last command gives us a
sequence from 1 to 2, but adds a negative sign in front of it.
...
The sleep data set has 62 rows.
...
The plot is comparing the total sleep of primates in comparison to other
species.
...
The issue in this data set is that there is a number of outliers, where
BrainWt\>1000.
...
TotalSleep decreases as BrainWt increases because there is a negative
estimate, the slope is negative. There is a significant p value greater
than .05.
